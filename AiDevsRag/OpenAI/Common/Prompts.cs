...
public static class Prompts
{
    public static string GetSystemPrompt(List<Result> searchResults)
    {
        ...
        string context = """
                         My (Alice) memories about the course: 
                         ###

                         """ +
                         string.Join("\n\n\n",
                             searchResults
                                 .Where(match => match.Payload.GetMetadata() is not null)
                                 .Select(match =>
                                 {
                                     Metadata metadata = match.Payload.GetMetadata()!;
                                     return
                                         $"Lesson: {metadata.Title} \n Fragment: {metadata.Header} Content: {metadata.Content}";
                                 })) +
                         "###";
